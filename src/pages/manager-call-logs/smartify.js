...
const query = searchQuery('searchManagerCallLogs')
const userCalls = state => state.managerCalls.results

const calls = callsSelector(query, userCalls)

const totalPages = state => state.managerCalls.totalPages

const selector = createStructuredSelector({ calls, totalPages })
...
const mapDispatchToProps = (dispatch, props) => ({
  ...
  loadCalls: (page, params = {}) =>
    dispatch(api.actions.managerCalls.get({ page }, params)),
  handleScroll: e => {
...
export default compose(
  ...
  lifecycle({
    componentDidMount() {
      ...
      const elem = document.getElementsByClassName('manager-call-logs-table')[1]
        .parentNode
