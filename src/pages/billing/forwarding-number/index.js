...
const ForwardingNumber = ({
  ...
  <div className="rounded-card">
    ...
    <OutboundNumberWrapper>
      ...
      {currentUser.callApiNumber ? (
        <div>
          <NumberHelpText>
            <VectorIcon name="call" />
            <span>Call API Number</span>
          </NumberHelpText>
          <OutboundNumber>{currentUser.callApiNumber.number}</OutboundNumber>
          <FlatButton label="call me" secondary onClick={callApi} />
        </div>
      ) : (
        <div>
          <FlatButton
            label="Create Call API Number"
            onClick={createCallApiNumber}
            secondary
          />
        </div>
      )}
