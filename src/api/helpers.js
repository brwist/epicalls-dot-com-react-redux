...
function setTotalPages(key, dispatch) {
  return response => {
    const payload = response.headers.get('total-pages')
    if (payload) {
      dispatch({ type: SET_TOTAL_PAGES, key, payload })
    }
    return response
  }
}

function setTotalCount(key, dispatch) {
  return response => {
    const payload = response.headers.get('total-count')
    if (payload) {
      dispatch({ type: SET_TOTAL_COUNT, key, payload })
    }
    return response
  }
}
  ...
  return (key, resource, fetchConfig = {}) => {
    const get = (params, config) => (dispatch, getState) => {
      ...
      return fetch(url, options)
        .then(checkResponse)
        .then(setTotalPages(key, dispatch))
        .then(setTotalCount(key, dispatch))
        .then(toJSON)
        .then(makeSuccessAction(key, dispatch, config))
        .then(
          fetchConfig.handleSuccess &&
            (data => fetchConfig.handleSuccess({ dispatch, data })),
        )
        .catch(
          fetchConfig.handleFailed &&
            (error => fetchConfig.handleFailed({ dispatch, error })),
        )
    }
  ...
  return (state = DEFAULT_STATE, action) => {
    ...
    if (action.type === SUCCESS_TYPE && action.key === key) {
      if (action.merge) {
        return {
          ...state,
          results: uniq(state.results, action.data),
          status: 'pending',
        }
      }
      ...
      return {
        ...state,
        results: action.data.results || action.data,
        meta: action.data.meta || state.meta,
        status: 'pending',
      }
    }

    if (action.type === SET_TOTAL_COUNT && action.key === key) {
      return {
        ...state,
    ...
    if (action.type === SET_TOTAL_PAGES && action.key === key) {
      return {
        ...state,
...
export function makePostActionCreator(endpoint, fetch) {
  return (url, options = {}) => (data, where = {}) => (dispatch, getState) => {
    ...
    const fetchOptions = {
      ...
      body: JSON.stringify(
        decamelizeKeys(data),
        options.whiteList && options.whiteList.map(decamelize),
      ),
    }
