  ...
  submit: data => {
    const request = dispatch(api.actions.passwordReset(data.id, data))
      .then(_ => {
        dispatch(
          api.actions.login({
            auth: { email: data.email, password: data.user.password },
          }),
        ).then(_ => history.push('/'))
        actions.reset(MODEL)
      })
      .catch(err =>
        Promise.reject({ commonErrors: translates[err.error] || err.error }),
      )
...
export default compose(
  ...
  lifecycle({
    componentDidMount() {
      ...
      this.props.change(`${MODEL}.id`, id)
      this.props.change(`${MODEL}.email`, email)
    },
  }),
)
